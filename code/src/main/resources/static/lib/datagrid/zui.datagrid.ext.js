...
/**
 * dailey.dai@openthinks.com modification 0.1 
 */
...
(function($, undefined) {
    ...
    var DataGrid = function(element, options) {
		...
		//dailey.dai@openthinks.com rmove ignore delay
        that.render();
        ...
        if ($.fn.searchBox) {
            ...
            if($searchBox)  {
            	//dailey.dai@openthinks.com add flag for auto register onSearchChange event
            	var sOptions = {};
            	if(options.autoOnSearchChange && options.autoOnSearchChange==true){
            		sOptions.onSearchChange = function (searchString) {
                        that.search(searchString);
                    }
            	}
                that.searchbox = $searchBox.searchBox(sOptions);
            }
        }
    };
    ...
    DataGrid.prototype.getFilterParams = function() {
        ...
        //dailey.dai@openthinks.com add extend filter params method
        if(that.options.extendFilterParams && $.isFunction(that.options.extendFilterParams)){
        	that.options.extendFilterParams(params);
        }
        return params;
    };
...
//      var dataId = [params.page, params.recPerPage, params.search, params.sortBy, params.order].join('&');
		//dailey.dai@openthinks.com build dataId as whole parametes
        var paramArray = [];
        for(var vname in params){
			paramArray.push(params[vname])        	
        }
        var dataId = paramArray.join('&');
    ...
    DataGrid.prototype.getCell = function(rowIndex, colIndex) {
        ...
        if (rowIndex > 0) {
            ...
            if (valueOperator && valueOperator.getter) {
            	//dailey.dai@openthinks.com add dataValue property
    ...
    DataGrid.DEFAULTS = {
        ...
        // auto bind search onSearchChange event
